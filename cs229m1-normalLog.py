...
oodDataVectors = pd.read_csv('oodVectors-200.csv')
...
print('oodLabels')
oodVectorsPost = pd.DataFrame(index=np.arange(337197), columns=np.arange(200))
for i in range(337197):
...
true_file = open('true_pred_proba.txt', 'w')
weak_file = open('weak_pred_proba.txt', 'w')
trueOOD_file = open('trueOOD_pred_proba.txt', 'w')
weakOOD_file = open('weakOOD_pred_proba.txt', 'w')
...
filenameTrue = 'finalized_model_true.sav'
pickle.dump(model_true, open(filenameTrue, 'wb'))
filenameWeak = 'finalized_model_weak.sav'
